...
namespace Affirma.ThreeSharp.Query
{
    ...
    public class ThreeSharpQuery : IThreeSharp
    {
		...
		/// <summary>
        /// Streams an range from and object down from a bucket
        /// </summary>
		public ObjectGetResponse ObjectGetRange(ObjectGetRangeRequest request)
        {
            return Invoke<ObjectGetResponse>(request);
        }
        ...
        private HttpWebRequest ConfigureWebRequest(Request request)
        {
            ...
            AddHeaders(httpWebRequest, request.ExtraData);
